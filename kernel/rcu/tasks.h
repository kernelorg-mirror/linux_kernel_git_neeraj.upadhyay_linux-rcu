...
static bool rcu_tasks_dynticks_in_eqs(int snap)
{
...
static void rcu_tasks_pertask(struct task_struct *t, struct list_head *hop)
{
	if (t != current && rcu_tasks_is_holdout(t)) {
		if (is_idle_task(t)) {
			...
			if (rcu_tasks_dynticks_in_eqs(rtpcp->dynticks_snap))
				return;
		}
 ...
 * state since rcu_tasks_dynticks_snap() returned the specified snapshot.
 */
static bool rcu_tasks_dynticks_in_eqs_since(int cpu, int snap)
{
	/*
			       ...
			       bool needreport, bool *firstreport)
{
	...
	if (!READ_ONCE(t->rcu_tasks_holdout) ||
	    t->rcu_tasks_nvcsw != READ_ONCE(t->nvcsw) ||
	    (is_idle_task(t) && rcu_tasks_dynticks_in_eqs_since(cpu, rtpcp->dynticks_snap)) ||
